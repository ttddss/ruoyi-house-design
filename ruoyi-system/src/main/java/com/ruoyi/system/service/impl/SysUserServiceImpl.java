...
public class SysUserServiceImpl implements ISysUserService
{
    ...
    public String importUser(List<SysUser> userList, Boolean isUpdateSupport, String operName)
    {
        ...
        for (SysUser user : userList)
        {
            ...
            {
                // 验证是否存在这个用户
                SysUser u = userMapper.selectUserByUserName(user.getPhonenumber());
                ...
                else if (isUpdateSupport)
                {
                    if (u.isAdmin()) {
                        failureNum++;
                        failureMsg.append("<br/>" + failureNum + "、管理员账号不能修改 ");
                        continue;
                    }
                    BeanValidators.validateWithException(validator, user);
                    checkUserAllowed(u);
    ...
    @Override
    public void unlockAuth(Long authId) {
        SysAuthUser authUser = userMapper.selectAuthUserListByAuthId(authId);
        userMapper.deleteUserById(authUser.getUserId());
        userMapper.deleteAuthUser(authId);
    }
